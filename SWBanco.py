...
class CreateContaWindow(QWidget):
    ...
    def function_add_new_conta(self):
        ...
        DataBase.data_base_add_conta(self.client_id, 
                                     self.input_saldo.value(), 
                                     self.input_limite_saques.value())
        print("Conta criada com sucesso!")
        contas = DataBase.data_base_get_contas_by_id(self.client_id)
        for conta in contas:
            print(conta.id)
            print(conta.saldo)
...
class ListClientsWindow(QWidget):
    ...
    def criar_conta(self, cliente):
        ...
        self.main_window = CreateContaWindow(cliente_id)
        self.main_window.setGeometry(100, 100, 600, 400)
        self.main_window.show()
        self.close()
    ...
    def listar_contas(self, cliente):
        ...
        contas = DataBase.data_base_get_contas_by_id(cliente.id)
        if len(contas) != 0:
            for conta in contas:
                print(conta.saldo)
            self.main_window = ListContasWindow(cliente.id)
            self.main_window.setGeometry(100, 100, 600, 400)
            self.main_window.show()
            self.close()
        else:
...
class ListContasWindow(QWidget):
    def __init__(self, client_id):
        ...
        self.setWindowTitle("Clients List Window")
        self.client_id = client_id
        cliente = DataBase.data_base_get_client_by_id(self.client_id)
    ...
    def excluir_conta(self, conta):
        ...
        self.main_window = ListContasWindow(self.client_id)
        self.main_window.setGeometry(100, 100, 600, 400)
        self.main_window.show()
        self.close()
    ...
    def realizar_transacao(self, conta):
        ...
        self.main_window = RealizarTranzactionWindow(self.client_id, conta.id)
        self.main_window.setGeometry(100, 100, 600, 400)
        self.main_window.show()
        self.close()
...
class RealizarTranzactionWindow(QWidget):
    def __init__(self, client_id, conta_id):
        super().__init__()
        self.setWindowTitle("Clients List Window")
        self.client_id = client_id
    ...
    def function_conta_sacar(self):
        print("Sacar!!")
        valor = -1 * int(self.input_valor.value())
        print("valor fora da function: ", valor)
        DataBase.data_base_realizar_transacao(self.conta_id, valor)
        conta = DataBase.data_base_get_conta_by_conta_id(self.conta_id)
        print(conta.saldo)
        print("---------------------------")

    def function_conta_depositar(self):
        print("Depositar!!")
        DataBase.data_base_realizar_transacao(self.conta_id, self.input_valor.value())
        conta = DataBase.data_base_get_conta_by_conta_id(self.conta_id)
        print(conta.saldo)
        print("---------------------------")
